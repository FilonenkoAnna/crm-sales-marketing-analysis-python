...
1. Run notebooks `01`–`05` to produce the cleaned CSV exports in `data/processed/`.
2. Create a local PostgreSQL database (e.g. `crm_project`).
3. Run the loading script above (adjust the connection URL to your local credentials).
4. Open any file in `queries/` in your SQL client of choice and run it against the loaded database.

No database file or credentials are stored in this repository.
